...
const DeviceProprtyGeoPointPage = () => {
  ...
  useEffect(() => {
    const fetchDeviceGeoPoints = async () => {
      try {
        ...
        const response = await axios.get(`${API_BASE_URL}/propertygeolocations/customer/${customer_id}/`);
  ...
  const rows = deviceGeoPoints.map((deviceGeoPoint) => [
    <Link key={deviceGeoPoint.property_id} to={`/property/survey-details/${deviceGeoPoint.property_id}`}
    >{deviceGeoPoint.property_name}</Link>,
  ...
  const handleEdit = (rowIndex) => {
    ...
    navigate('/device-property-geopoint/add-devicegeopoint', {
  ...
  const confirmDelete = async () => {
    try {
      await axios.delete(
        `${API_BASE_URL}/devicepropertygeopoint/delete/${deviceGeoPoints[deleteIndex].id}/`
      );
    ...
    <Container fluid className="section">
      ...
      <Row className="g-3">
        <Col lg={12} className="d-flex justify-content-between align-items-center">
          ...
          <AddButton buttonText={'Add Device Geolocation'} path="/device-property-geopoint/add-devicegeopoint" />
        </Col>
        <Col lg={12}>
...
export default withAuthRedirect(DeviceProprtyGeoPointPage);
